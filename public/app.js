...
// Report generation takes ~15-25s (up to 9 Claude calls plus an image
// call). There's no real server-pushed progress, so this cycles through a
...
// the actual work happening (research.js does genuinely read the CV, read
// the JD, then research the company, reviews, etc.).
...
function startGenerationProgress() {
  ...
  if (!el) return () => {};
  let i = 0;
  el.classList.add("gen-progress");
  el.textContent = GENERATION_STEPS[0];
  const timer = setInterval(() => {
    i = (i + 1) % GENERATION_STEPS.length;
    el.textContent = GENERATION_STEPS[i];
  }, 2200);
  return () => {
    clearInterval(timer);
    el.classList.remove("gen-progress");
  };
}
