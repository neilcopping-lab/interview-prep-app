...
// ---------------------------------------------------------------------
// Assemble the report. A report now involves up to 9 Claude calls (5 of
// them web-search-grounded). Firing all 9 in one Promise.all works, but
// under load it's the fastest way to trip a per-minute rate limit on a
// lower API tier — claudeText/claudeJSON already retry on 429s, but as
// extra headroom the web-search-heavy calls run in a smaller first wave
// so they're not all racing the JD/CV-based calls for the same quota at
// once. Total time added is small since each wave still runs in parallel.
// ---------------------------------------------------------------------
...
async function generateReport(input) {
  ...
  // Cover art runs on OpenAI, not Claude, so it doesn't compete with the
  // Anthropic web-search calls for quota — safe to kick off alongside the
  // first wave rather than needing its own stagger.
  const [research, sentiment, social, cover] = await Promise.all([
  ...
  const [market, news, challenges, gaps, pitch, starAnswers] = await Promise.all([
