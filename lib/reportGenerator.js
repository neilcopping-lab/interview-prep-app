...
const { hasAnthropic, claudeText, claudeJSON } = require("./aiClients");
...
async function companyResearch(companyName) {
  ...
  if (!result) return companyResearchFallback(companyName);
  return { snapshot: result.text, sources: result.sources };
}
...
function employeeSentimentFallback(companyName) {
  return {
    summary: `[AI UPGRADE POINT] A summary of what current and former employees say about working at `
      + `"${companyName}" (Glassdoor, Google, Indeed reviews) would appear here — common praise, common `
      + `complaints, and anything worth probing gently in the interview. Wire ANTHROPIC_API_KEY with web `
      + `search enabled to populate this automatically.`,
    sources: [],
  };
}

async function employeeSentiment(companyName) {
  if (!companyName || !companyName.trim()) return employeeSentimentFallback(companyName);
  if (!hasAnthropic()) return employeeSentimentFallback(companyName);
  ...
  const result = await claudeText(
    ...
    `Write a candid 120-180 word briefing (plain prose, no headers) covering: recurring themes in what ` +
    `people praise, recurring themes in what people criticise, and roughly how the company seems to be ` +
    `rated overall if a rating is available. Be balanced — don't sand off real criticism, and don't ` +
    `exaggerate a handful of angry reviews into a pattern either. This is meant to help someone walk in ` +
    `with realistic expectations and maybe a sharp question or two, not to talk them out of the job. If ` +
    `you can't find genuine review data for this company, say so plainly rather than guessing.`,
    { webSearch: true, maxTokens: 700 }
  );

  if (!result) return employeeSentimentFallback(companyName);
...
function socialMediaFallback(companyName) {
  return {
    summary: `[AI UPGRADE POINT] A rundown of "${companyName}"'s social media presence and recent activity `
      + `(LinkedIn, Instagram, X/Twitter, etc.) would appear here — what they post about, tone of voice, `
      + `and anything recent worth mentioning in interview. Wire ANTHROPIC_API_KEY with web search enabled `
...
async function socialMedia(companyName) {
  if (!companyName || !companyName.trim()) return socialMediaFallback(companyName);
  if (!hasAnthropic()) return socialMediaFallback(companyName);

  const result = await claudeText(
    `Search the web for "${companyName}"'s social media presence — LinkedIn, Instagram, X/Twitter, ` +
    `TikTok, Facebook, whichever they're actually active on. Write a 120-180 word briefing (plain prose, ` +
    `no headers) covering: which platforms they're genuinely active on, what tone/personality comes ` +
    `through in their posts, and 1-2 specific recent posts or campaigns worth knowing about (name them ` +
    `specifically if you can find them — a launch, an award, a hiring push, a rebrand, anything concrete). ` +
    `This is so a candidate can naturally reference something current and show they've actually looked. ` +
    `If you can't find much genuine social activity for this company, say so plainly rather than padding ` +
    `it out.`,
    { webSearch: true, maxTokens: 700 }
  );

  if (!result) return socialMediaFallback(companyName);
  return { summary: result.text, sources: result.sources };
}
...
function marketIntelligenceFallback(companyName) {
  return {
    summary: `[AI UPGRADE POINT] A briefing on market and sector trends relevant to "${companyName}" and `
      + `this role would appear here — what's happening in their industry right now, and how it might `
      + `shape what the interviewer cares about. Wire ANTHROPIC_API_KEY with web search enabled to `
...
async function marketIntelligence(companyName, jobDescription) {
  if (!companyName || !companyName.trim()) return marketIntelligenceFallback(companyName);
  if (!hasAnthropic()) return marketIntelligenceFallback(companyName);

  const result = await claudeText(
    `Search the web for current market and sector trends relevant to "${companyName}" and the role ` +
    `described below. Write a 150-220 word briefing (plain prose, no headers) covering: what's currently ` +
    `happening in their industry or sector (economic pressure, regulatory change, technology shift, ` +
    `consumer behaviour change — whatever is genuinely relevant), and how that context might shape what ` +
    `this interviewer actually cares about right now. The goal is for the candidate to be able to speak ` +
    `to "what's happening in the market" with real substance instead of generic statements. If you can't ` +
    `find much sector-specific information, say so plainly rather than guessing.\n\n` +
    `JOB DESCRIPTION:\n${jobDescription || "(not provided)"}`,
    { webSearch: true, maxTokens: 800 }
  );

  if (!result) return marketIntelligenceFallback(companyName);
  return { summary: result.text, sources: result.sources };
}
...
function recentNewsFallback(companyName) {
  return {
    summary: `[AI UPGRADE POINT] A summary of recent news and press coverage of "${companyName}" would `
      + `appear here. Wire ANTHROPIC_API_KEY with web search enabled to populate this automatically.`,
    sources: [],
  };
}

async function recentNews(companyName) {
  if (!companyName || !companyName.trim()) return recentNewsFallback(companyName);
  if (!hasAnthropic()) return recentNewsFallback(companyName);

  const result = await claudeText(
    `Search the web for recent news and press coverage of "${companyName}" — roughly the last 6-12 ` +
    `months. Write a 120-180 word briefing (plain prose, no headers) on what's actually happened: ` +
    `funding, leadership changes, launches, awards, restructuring, controversies, whatever is genuinely ` +
    `there. Name specific things (dates, people, figures) where you find them rather than speaking in ` +
    `vague terms. If there's genuinely little recent press coverage of this company, say so plainly ` +
    `rather than inventing news.`,
    { webSearch: true, maxTokens: 700 }
  );

  if (!result) return recentNewsFallback(companyName);
  return { summary: result.text, sources: result.sources };
}
...
async function generateReport(input) {
  ...
  const [gaps, research, sentiment, social, market, news, pitch, starAnswers] = await Promise.all([
